...
state_machine::state_machine(entt::registry& reg, system_dispatcher& disp,
                             const std::shared_ptr<game_state>&& initial_state)
    : reg{reg}, disp{disp}, running{true}, state_stack{
                                                   std::move(initial_state)} {
    state_stack.back()->on_start(reg, disp);
}

state_machine::~state_machine() {
    while (!state_stack.empty()) {
        auto state = state_stack.back();
        state->on_stop(reg, disp);
...
void state_machine::update() {
    auto& state = state_stack.back();
    auto trans = state->update(reg, disp);
