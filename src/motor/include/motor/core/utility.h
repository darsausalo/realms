...
constexpr std::string_view nameof_type() {
    ...
    static constexpr char enum_prefix[] = "enum ";
    static constexpr std::size_t enum_prefix_size = sizeof(enum_prefix) - 1;
    ...
    } else if (auto n = type_name.find(enum_prefix);
               n != std::string_view::npos) {
        type_name = type_name.substr(enum_prefix_size);
    }
