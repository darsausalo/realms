...
myml_table_t* myml_find_subtable(const myml_table_t* table,
    ...
    for (int i = 0; i < table->size; i++) {
        ...
        if (strncmp(value->key, key_path, len) == 0 &&
            ...
            return partial
                       ? myml_find_subtable(value->table, &key_path[len + 1])
                       : value->table;
        }
    }
...
myml_error_t myml_parse(myml_table_t* table, const char* source) {
    ...
    myml_stack_push(&ctx.levels, table);
    ...
    for (int i = 0; source[i] != '\0'; i++) {
        ...
        if (curc == '\n') {
            ...
            if (ctx.levels.size <= level) {
                return myml_error(&ctx, "bad indent");
            }
            ...
            if (key_start != -1) {
                if (key_stop == -1) {
                    return myml_error(&ctx, "expected colon");
                }
...
static void myml_merge_base(myml_table_t* table, myml_table_t* base) {
    ...
    for (int i = 0; i < base->size; i++) {
        ...
        if (base_value->type == MVT_STRING) {
            if (!value && strncmp(base_value->key, MYML_INHERITS,
                                  MYML_INHERITS_SIZE) != 0) {
...
static myml_error_t myml_take_inherit_from(myml_table_t* table,
    ...
    for (int i = 0; i < table->size; i++) {
        ...
        if (strncmp(table->values[i].key, MYML_INHERITS, MYML_INHERITS_SIZE) ==
            0) {
            myml_value_t value;
